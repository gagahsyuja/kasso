<script lang="ts">
    ...
    const load = async (): Promise<Array<any>> => {

        const db = await Database.load("sqlite:database.db");

        return await db.select('SELECT * FROM transactions INNER JOIN categories ON transactions.category_id = categories.id WHERE transactions.id IS NOT NULL');
    ...
    const loadWithFilter = async (type: String, method: String, category: String,
        ...
        let query = 'SELECT * FROM transactions INNER JOIN categories ON transactions.category_id = categories.id WHERE transactions.id IS NOT NULL';

        if (type !== "All") query = `${query} AND transactions.type = "${type}"`;
        // if (method !== "All") query = `${query} AND method = ${method}`;
...
<Main>
    ...
    {#await getCategories() then categories}
        ...
        {/if}
    {/await}
    {#await loadWithFilter(filter.type, "All", filter.category, filter.date.from, filter.date.to) then transactions}
